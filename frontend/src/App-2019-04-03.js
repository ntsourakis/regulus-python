...
class App extends Component {
	...
	match(response) {
		...
		axios.post('http://127.0.0.1:8000/dante/message/', {"message":["match", response.toLowerCase()], "state":this.state.st}, { headers: { Authorization: AuthStr } })
		.then(response => {
			// If request is good...
  ...
  getLessons() {
	...
	render() {
		...
		const { transcript, resetTranscript, browserSupportsSpeechRecognition, finalTranscript } = this.props
		...
		<div>
			...
			<form onSubmit={this.responseSubmit}>
				<label>
					Response:
					<input type="text" value={finalTranscript} onChange={this.responseChange} />
				</label>
				<input type="submit" value="Submit" />
			</form>
